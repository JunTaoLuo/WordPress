...
- (void)setUsername:(NSString *)username password:(NSString *)password success:(void (^)())success failure:(void (^)(NSError *error))failure {
    ...
    [self authenticateWithSuccess:^{
        ...
        if (error) {
            ...
            [AppDelegate sharedWordPressApp].isWPcomAuthenticated = YES;
            [[AppDelegate sharedWordPressApp] registerForPushNotifications];
...
- (void)signOut {
    ...
    [[NSUserDefaults standardUserDefaults] synchronize];
    [AppDelegate sharedWordPressApp].isWPcomAuthenticated = NO;
    [[AppDelegate sharedWordPressApp] unregisterApnsToken];
    self.username = nil;
    self.password = nil;
